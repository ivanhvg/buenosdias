// src/lib/texts/octubre.ts
import type { DailyText } from '@/lib/texts';

// Aquí puedes añadir todos los textos para el mes de octubre.
// El sistema los cargará automáticamente según el día.
const dailyTexts: DailyText[] = [
  {
    level: 'infantil-primaria',
    day: 1,
    text: '**La casa sobre roca**. Lc 6,47-49\n\n“Todo el que viene a mí, escucha mis palabras y las pone en práctica, os voy a decir a quién se parece: se parece a uno que edificó una casa: cavó, ahondó y puso los cimientos sobre roca; vino una crecida, arremetió el río contra aquella casa, y no pudo derribarla, porque estaba sólidamente construida. El que escucha y no pone en práctica se parece a uno que edificó una casa sobre tierra, sin cimiento; arremetió contra ella el río, y enseguida se derrumbó desplomándose, y fue grande la ruina de aquella casa.”'
  },
  {
    level: 'secundaria',
    day: 1,
    text: 'Comienza un nuevo mes, una página en blanco. ¿Qué pequeño objetivo realista te puedes marcar para estas próximas semanas? Piensa en algo que dependa de ti.'
  },
  {
    level: 'infantil-primaria',
    day: 2,
    text: '**Por la familia**\n\nSeñor, te pedimos por todas las familias del mundo, para que vivan unidas en el amor, el respeto y la comprensión.\n\nTe lo pedimos señor.'
  },
  {
    level: 'secundaria',
    day: 2,
    text: 'La empatía es la capacidad de ponerse en el lugar del otro. Hoy, antes de juzgar la acción de un compañero o amigo, intenta pensar qué puede estar sintiendo o por qué actúa de esa manera.'
  },
  {
    level: 'infantil-primaria',
    day: 3,
    text: '**“GRACIAS POR LA CREACIÓN”**\n\nGracias, Señor Jesús, por ser el creador del universo, el sol, la luna, las estrellas y todos los seres vivos. Te agradezco por haberme hecho a tu imagen y semejanza, dotándome de mente para pensar, corazón para amar y voluntad para elegir la belleza. Agradezco profundamente tu amor incondicional. Y hoy de manera especial te doy gracias por San Francisco que nos ayuda a descubrirte. Amén.'
  },
  {
    level: 'infantil-primaria',
    day: 6,
    text: 'Adriana y Daniel visitan un museo y descubren un cuadro de don Federico que cambia de paisaje. Al mirarlo, son transportados a un bosque encantado donde un búho les explica que el cuadro lleva a quienes tienen el corazón puro y les enseña a valorar la naturaleza. Tras aprender de criaturas y plantas, prometen proteger el entorno y regresan al museo. Desde entonces, se convierten en defensores y narradores de esa magia, recordando a todos la responsabilidad de cuidar el mundo.'
  },
  {
    level: 'infantil-primaria',
    day: 7,
    text: '**Informativo BC**\n\nUn niño de 12 años llamado Francisco Vera ha grabado un vídeo muy emotivo para promover el cuidado del medio ambiente. En él, habla sobre la importancia de proteger la Tierra, plantar árboles, reducir la basura, usar menos plástico y cuidar los recursos naturales.\n\nEs una noticia positiva porque muestra que los niños también pueden ser agentes de cambio, tener voz, y motivar a otros para hacer cosas buenas por el planeta.\n\nDato curioso el día de La Tierra: es el 22 de abril.\n\nVídeo: [https://www.youtube.com/watch?v=NmQOjnQSbQ8](https://www.youtube.com/watch?v=NmQOjnQSbQ8)'
  },
  {
    level: 'infantil-primaria',
    day: 8,
    text: '**El remiendo en paño viejo.** Lc 5,36\n\n“Les dijo también una parábola: «Nadie recorta una pieza de un manto nuevo para ponérsela a un manto viejo; porque, si lo hace, el nuevo se rompe y al viejo no le cuadra la pieza del nuevo.”'
  },
  {
    level: 'infantil-primaria',
    day: 9,
    text: '**Por el colegio**\n\nTe presentamos, Señor, a nuestro colegio, a los profesores, estudiantes y personal que aquí trabajan, para que juntos sigamos creciendo.\n\nTe lo pedimos señor'
  },
  {
    level: 'infantil-primaria',
    day: 10,
    text: '**“GRACIAS POR ESTE DÍA”**\n\nQuerido Dios, gracias por este hermoso día que me has dado. Gracias por el sol que brilla, por mis amigos con quienes juego y por mi amorosa familia. Ayúdame a recordar siempre tus bendiciones y a ser agradecido por cada cosa, grande o pequeña, en mi vida. Amén.'
  },
  {
    level: 'infantil-primaria',
    day: 13,
    text: 'El profesor VegaX crea a RobiX, un robot curioso. Al oír a SofíaX hablar de sueños, RobiX descubre que no puede soñar como los humanos, pero aprende que los sueños son deseos que dan esperanza y dirección. Con SofíaX comprende que soñar es proponerse metas que ayudan a otros. Decide que su “sueño” será apoyar a las personas para que alcancen los suyos. Juntos, usan la tecnología de RobiX y la empatía de SofíaX para mejorar su comunidad. Aunque RobiX no sueña dormido, vive la esencia de los sueños cada vez que provoca una sonrisa.'
  },
  {
    level: 'infantil-primaria',
    day: 14,
    text: '**Informativo BC**\n\nEl amor de un padre no conoce límites, y esta historia lo demuestra. Un ingeniero decidió poner todo su conocimiento y creatividad al servicio de su hijo, quien había perdido la capacidad de caminar.\n\nDespués de meses de esfuerzo, construyó un exoesqueleto robotizado que permitió al pequeño dar nuevamente sus primeros pasos. El emotivo momento fue recibido entre lágrimas de alegría por su familia y médicos.\n\nVídeo: [https://www.youtube.com/watch?v=pCAm4fsz0gA&t=7s](https://www.youtube.com/watch?v=pCAm4fsz0gA&t=7s)'
  },
  {
    level: 'infantil-primaria',
    day: 15,
    text: '**La lámpara en el candelero**. Lc 8,16\n\n“Nadie que ha encendido una lámpara, la tapa con una vasija o la mete debajo de la cama, sino que la pone en el candelero para que los que entren vean la luz.”'
  },
  {
    level: 'infantil-primaria',
    day: 16,
    text: '**Por los ancianos**\n\nTe pedimos por los abuelos y ancianos, para que nunca se sientan solos y podamos valorar su experiencia y sabiduría.\n\nTe lo pedimos señor'
  },
  {
    level: 'infantil-primaria',
    day: 17,
    text: '**“GRACIAS POR MIS AMIGOS”**\n\nSeñor Jesús, te agradezco por las muchas bendiciones en mi vida, especialmente por tu constante presencia como un amigo infalible. Amén.'
  },
  {
    level: 'infantil-primaria',
    day: 20,
    text: 'Nano, un niño fascinado por lo diminuto, recibe el microscopio de su abuelo y, junto a su gato parlante Miau, se encoge y llega a la Microciudad de Bacti. Allí, la Reina Bacti les cuenta que la contaminación del “mundo de los gigantes” está desequilibrando su ecosistema. Con la ayuda de Viri, un virus que busca su lugar, atraviesan la Jungla de Hongos y descubren un conducto de desechos químicos. Nano propone usar microorganismos limpiadores como filtro natural; todos colaboran y el equilibrio se restaura. Humanos y microseres sellan una alianza, y Nano comprende que en lo más pequeño habitan un gran poder y una gran responsabilidad.'
  },
  {
    level: 'infantil-primaria',
    day: 21,
    text: '**Informativo BC**\n\nEn Texas, un abuelo llamado Eugene Bostick encontró una manera muy especial de alegrar la vida de los perritos callejeros. Después de rescatar a varios, decidió construirles un “tren de perritos”: un tractor que arrastra pequeños vagones hechos con barriles de plástico, adaptados con asientos y ventanas.\n\nVarias veces por semana, Eugene engancha los vagones y pasea a sus pasajeros peludos por el vecindario y el campo. Los perritos, felices y con las orejas al viento, disfrutan de cada recorrido como si fuera una gran aventura.\n\nVideo: [https://youtube.com/shorts/Ha2VoHQoXuQ?feature=shared](https://youtube.com/shorts/Ha2VoHQoXuQ?feature=shared)'
  },
  {
    level: 'infantil-primaria',
    day: 22,
    text: '**Los niños en la plaza**. Lc 7, 31-35\n\n“¿A quién, pues, compararé los hombres de esta generación? ¿A quién son semejantes? Se asemejan a unos niños, sentados en la plaza, que gritan a otros aquello de: “Hemos tocado la flauta y no habéis bailado, hemos entonado lamentaciones, y no habéis llorado”.'
  },
  {
    level: 'infantil-primaria',
    day: 23,
    text: '**Por los amigos**\n\nTe pedimos, Señor, por nuestros amigos, para que cultivemos la amistad verdadera, el apoyo mutuo y la alegría de compartir la vida juntos.\n\nTe lo pedimos señor'
  },
  {
    level: 'infantil-primaria',
    day: 24,
    text: '**“GRACIAS POR LA FAMILIA”**\n\nSeñor, gracias por la presencia de mis padres en mi vida, quienes me aman y buscan lo mejor para mí. Te pido que los mantengas mucho tiempo a mi lado y me hagas capaz de contribuir a su felicidad. Ayúdanos a comprendernos y amarnos, siguiendo el ejemplo de la Sagrada Familia de Nazareth. Amén.'
  },
  {
    level: 'infantil-primaria',
    day: 27,
    text: 'Mila visita un museo de ciencias y, junto a Neo, un robot guía, activa el Compás Cósmico que los transporta a un valle donde los elementos son personajes. Oxígeno e Hidrógeno les muestran cómo formar agua; luego, con Carbono, equilibran una reacción para crear dióxido de carbono. Custodiados por Oro, superan pruebas en una caverna hasta hallar el Cristal de la Sabiduría y comprenden su mensaje: la química trata de cómo los elementos, al unirse, logran algo mayor que por separado. Vuelven al museo sabiendo que aprender es combinar para mejorar el mundo.'
  },
  {
    level: 'infantil-primaria',
    day: 28,
    text: '**“Un niño de 10 años protege a su hermana en un accidente y moviliza a toda su comunidad”**\n\nEn Filadelfia, André Howard III (10 años) resultó gravemente herido al cubrir con su cuerpo a su hermana pequeña cuando cayó el fuselaje de una avioneta (31 de enero de 2025). Su valentía conmovió a vecinos y compañeros del colegio, que organizaron una recaudación para ayudar a su familia con los gastos médicos: superaron los 117 160 dólares en donaciones. Una historia de coraje y amor fraternal que demuestra cómo un gesto de un niño puede inspirar a toda una ciudad.'
  },
  {
    level: 'infantil-primaria',
    day: 29,
    text: '**Los dos deudores**. Lc 7, 41-43\n\n“Un prestamista tenía dos deudores: uno le debía quinientos denarios y el otro cincuenta. Como no tenían con qué pagar, los perdonó a los dos. ¿Cuál de ellos le mostrará más amor?». Respondió Simón y dijo: «Supongo que aquel a quien le perdonó más». Y él le dijo: «Has juzgado rectamente»”.'
  },
  {
    level: 'infantil-primaria',
    day: 30,
    text: '**Por la paz**\n\nSeñor, derrama tu paz sobre el mundo, especialmente en los lugares donde hay violencia, injusticia o guerra, para que todos aprendamos a vivir como hermanos.\n\nTe lo pedimos señor'
  },
  {
    level: 'infantil-primaria',
    day: 31,
    text: '**“GRACIAS POR LOS ANIMALES”**\n\nQuerido Dios, gracias por el hermoso mundo que nos has dado, lleno de árboles, flores, ríos y montañas, y por todos los animales, desde los pequeños insectos hasta los grandes elefantes. Ayúdanos a todos a cuidar de tu creación, para que siga siendo un hogar hermoso para todas las criaturas. Amén.'
  }
];

export default dailyTexts;
